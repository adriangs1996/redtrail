...
3. **Execute each test:**
   ...
   - LLM tests (`.llm.sh`): 3 runs, PASS if 2/3 succeed (majority vote)
...
5. **Loop (max 5 iterations):**
   ...
   - **Decision:**
     - **Target passes + no regressions** → commit with descriptive message, exit 0
     - **Regression** (a previously-passing test now fails) → `git checkout -- src/` (revert Claude's changes), append to `attempts_log`:
       ```
...
- `eval/` directory is protected — changes reverted after each Claude invocation (same as eval loop)
