...
SKIP_COUNT=$("$RT" query "SELECT count(*) as cnt FROM commands WHERE command LIKE '%skipped-by-env%'" --json 2>/dev/null)
...
NORMAL_COUNT=$("$RT" query "SELECT count(*) as cnt FROM commands WHERE command LIKE '%captured-normal%'" --json 2>/dev/null)
...
AFTER_COUNT=$("$RT" query "SELECT count(*) as cnt FROM commands WHERE command LIKE '%captured-after-skip%'" --json 2>/dev/null)
