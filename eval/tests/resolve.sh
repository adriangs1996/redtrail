...
# Should show the fix command (npm install or npm test as resolution)
echo "$OUTPUT" | grep -qi "npm" || {
  echo "FAIL: should show npm in resolution"
  echo "$OUTPUT"
  exit 1
}

# Should show fix success rate (worked N/3)
echo "$OUTPUT" | grep -q "/3" || {
  echo "FAIL: should show success rate with /3 denominator"
  echo "$OUTPUT"
  exit 1
}
