...
pub fn run(db: &impl SessionOps, session_id: &str, json: bool) -> Result<(), Error> {
    ...
    println!("Session: {}", summary["session_name"].as_str().unwrap_or(""));
    ...
    println!();
    println!("Hosts:   {} discovered", summary["hosts"].as_i64().unwrap_or(0));
    println!("Ports:   {} open", summary["ports"].as_i64().unwrap_or(0));
    println!("Creds:   {} found", summary["creds"].as_i64().unwrap_or(0));
    println!("Flags:   {} captured", summary["flags"].as_i64().unwrap_or(0));
    println!("Hyps:    {} pending, {} confirmed, {} refuted",
    ...
    println!("Noise:   {}/1.0", summary["noise_budget"].as_f64().unwrap_or(0.0));
