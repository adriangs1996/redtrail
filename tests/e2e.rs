...
fn test_full_pentest_workflow() {
    ...
    // 1. Init workspace
    let out = rt(&["init", "--target", "10.10.10.1", "--goal", "capture-flags", "--scope", "10.10.10.0/24"], dir);
    ...
    let out = rt(&["echo", "PORT STATE SERVICE\n22/tcp open ssh\n80/tcp open http"], dir);
    assert!(out.status.success());

    // 4. Add hosts and ports
    rt(&["kb", "add-host", "10.10.10.1", "--os", "Linux", "--hostname", "target"], dir);
    rt(&["kb", "add-port", "10.10.10.1", "22", "--service", "ssh", "--version", "OpenSSH 8.9"], dir);
    rt(&["kb", "add-port", "10.10.10.1", "80", "--service", "http", "--version", "nginx 1.18"], dir);
    ...
    // 7. Create hypotheses
    rt(&["hypothesis", "create", "SQLi in login form", "--category", "I", "--priority", "high", "--confidence", "0.8"], dir);
    rt(&["hypothesis", "create", "Debug console exposed on :80", "--category", "C", "--priority", "critical", "--confidence", "0.6"], dir);
    ...
    rt(&["hypothesis", "update", &hyp_id.to_string(), "--status", "confirmed"], dir);

    let confirmed = rt_json(&["hypothesis", "list", "--status", "confirmed", "--json"], dir);
    assert_eq!(confirmed.as_array().unwrap().len(), 1);

    // 9. Add evidence
    rt(&["evidence", "add", "--finding", "SQL error in response with single quote",
         "--hypothesis", &hyp_id.to_string(),
         "--severity", "critical",
         "--poc", "curl http://10.10.10.1/login -d 'user=admin\\''"], dir);
    ...
    rt(&["kb", "add-cred", "admin", "--pass", "admin123", "--service", "mysql", "--host", "10.10.10.1"], dir);
    rt(&["kb", "add-flag", "HTB{e2e_test_flag}", "--source", "/home/user/user.txt"], dir);
    ...
    // 11. Add access
    rt(&["kb", "add-access", "10.10.10.1", "admin", "user", "--method", "ssh"], dir);
    ...
    rt(&["report", "generate", "--output", report_path.to_str().unwrap()], dir);
    ...
    // 20. Notes
    rt(&["kb", "add-note", "SSH credentials found via SQLi dump"], dir);
